  ...
  const fetchHandler = async () => {
    try {
      const response = await fetch(
        `https://openlibrary.org/search.json?author=${props.author}&limit=10&fields=key,title,author_name,ratings_average,cover_edition_key,edition_key,first_publish_year,number_of_pages_median,subject&sort=rating`,
      );
      if (!response.ok) {
